...
class BaseNeuron(ABC):
    ...
    @abstractmethod
    def set_random_weights(self, multiplier=1):
        pass
...
class Linear(BaseNeuron):
    ...
    def set_random_weights(self, multiplier=1):
        self.bias = (np.random.random() - 0.5) * multiplier
        self.weights = np.asarray([(np.random.random() - 0.5) * multiplier for _ in range(len(self.weights))])
    ...
    def gradient(self, data, target):
        ...
        derror_dweights = np.multiply(derror_dprediction, dprediction_dweights)
        derror_dbias = derror_dprediction * dprediction_dbias

        return derror_dweights, derror_dbias

    def learning(self, data, target):
        derror_dweights, derror_dbias = self.gradient(data, target)
        self.weights = self.weights - np.multiply(self.learning_rate, derror_dweights)
...
class Neuron(Linear):
    ...
    def __init__(self, weights_number=1, bias=0., learning_rate=0.1, activation=sigmoid,
                 dactivation=dsigmoid):

        super().__init__(weights_number, bias, learning_rate)
    ...
    def gradient(self, data: np.ndarray, target):
        ...
        derror_dweights, derror_dbias = super().gradient(data, target)

        dprediction_dlayer = self.dactivation(super().prediction(data))  # vrai pour toute fonction

        derror_dweights *= dprediction_dlayer
        derror_dbias *= dprediction_dlayer
